package org.Kyoee01.PixelmonServer.listeners;
...
import org.Kyoee01.PixelmonServer.sever.Prefixes;
...
public class ForgeEventListener{
    ...
    @SubscribeEvent
    public void onUseExpCandyEvent(ExperienceGainEvent e) throws IOException {
        ...
        if (!e.isCanceled() && player != null){
            String name;
            if (e.pokemon.getPokemon().getNickname() == null){
            ...
            player.sendMessage(new StringTextComponent(ChatColor.translateAlternateColorCodes('&',
                    Prefixes.getServer() +"&a&l"+ name +" &f이(가) &e"+format.format(e.getExperience())+"EXP &f를 얻었습니다.")),
                    UUID.randomUUID());
        }
    }
}
